meta:
  id: utp
  title: BioWare UTP (Placeable) File Format
  license: MIT
  endian: le
  file-extension: utp
  xref:
    pykotor: vendor/PyKotor/Libraries/PyKotor/src/pykotor/resource/generics/utp.py
    wiki: vendor/PyKotor/wiki/GFF-UTP.md
doc: |
  UTP (Placeable) files are GFF-based format files that define placeable object templates including
  containers, furniture, switches, workbenches, and interactive environmental objects. Placeables can
  have inventories, be destroyed, locked, trapped, and trigger scripts. UTP files use the GFF
  (Generic File Format) binary structure with file type signature "UTP ".
  
  UTP files contain:
  - Root struct with placeable metadata:
    - TemplateResRef: Placeable template ResRef (unique identifier, max 16 characters, ResRef type)
    - LocName: Localized placeable name (LocalizedString/CExoLocString type)
    - Description: Placeable description (LocalizedString/CExoLocString type, deprecated, not used by game engine)
    - Tag: Placeable tag identifier (String/CExoString type, used for scripting references)
    - Comment: Developer comment string (String/CExoString type, toolset only, not used by game engine)
    - Conversation: Dialog ResRef (ResRef type, triggers dialog when placeable is used)
    - Faction: Faction identifier (UInt32, faction ID for placeable ownership)
    - Appearance: Appearance index (UInt32, index into placeables.2da, determines model)
    - Type: Placeable type category (UInt8/Byte, determines behavior: container, switch, generic, etc.)
  ...
  - Lock/unlock properties:
    ...
    - CloseLockDC: Security skill DC to lock placeable (UInt8/Byte, KotOR2 only)
  ...
  - Inventory system:
    - HasInventory: Placeable contains items (UInt8/Byte, 0 = no inventory, 1 = has inventory)
    - ItemList: List of items in inventory (List type, contains InventoryItem structs)
    - BodyBag: Container for corpse loot (UInt8/Byte, 0 = normal container, 1 = corpse container)
  - Interaction flags:
    - Plot: Plot-critical flag (UInt8/Byte, 0 = normal placeable, 1 = cannot be destroyed)
    - Static: Static geometry flag (UInt8/Byte, 0 = interactive, 1 = non-interactive)
    - Useable: Can be clicked/used (UInt8/Byte, 0 = cannot use, 1 = can use)
    - PartyInteract: Requires party member selection (UInt8/Byte, KotOR2 only)
  - Script hooks (all ResRef type):
    - OnUsed: Fires when placeable is used/clicked
    - OnOpen: Fires when placeable opens (containers)
    - OnClosed: Fires when placeable closes (containers)
    - OnDamaged: Fires when placeable takes damage
    - OnDeath: Fires when placeable is destroyed
    - OnDisarm: Fires when trap is disarmed (deprecated, always empty)
    - OnEndDialogue: Fires when conversation ends
    - OnFailToOpen: Fires when opening fails (KotOR2 only)
    - OnHeartbeat: Fires periodically
    - OnInvDisturbed: Fires when inventory changed
    - OnLock: Fires when placeable is locked (always empty in files)
    - OnMeleeAttacked: Fires when attacked in melee
    - OnSpellCastAt: Fires when spell cast at placeable (OnForcePower in PyKotor)
    - OnTrapTriggered: Fires when trap triggers (deprecated, always empty)
    - OnUnlock: Fires when placeable is unlocked (always empty in files)
  ...
  - Containers: Footlockers, crates, corpses. Have inventory (ItemList populated), can be locked, trapped, destroyed
  - Switches & Terminals: Trigger scripts or conversations. No inventory typically, Useable=1, Conversation or scripts set
  - Workbenches: Special placeable type for crafting. Opens crafting interface on use
  - Furniture: Non-interactive decoration. Static=1 or Useable=0, pure visual elements
  - Environmental Objects: Explosive containers, power generators. Can be destroyed with effects, often have HP and OnDeath scripts
  
  Instantiation Flow:
  1. Template Load: GFF parsed from UTP file
  2. Appearance Setup: Model loaded from placeables.2da
  3. Inventory Population: ItemList instantiated (if HasInventory=1)
  ...
  - BodyBag containers have special loot rules (corpses)
  ...
  References:
  - vendor/PyKotor/wiki/GFF-UTP.md
  - vendor/PyKotor/wiki/GFF-File-Format.md
  - vendor/PyKotor/Libraries/PyKotor/src/pykotor/resource/generics/utp.py:17-377
  ...
  gff_header:
    seq:
      - id: file_type
        ...
        size: 4
        doc: |
          File type signature. Must be "UTP " for placeable template files.
          Other GFF types: "GFF ", "DLG ", "ARE ", "UTC ", "UTI ", "UTD ", "UTM ", "GIT ", etc.
        valid: "UTP "
    ...
    seq:
      - id: name
        ...
        doc: |
          ...
          Common UTP field names: "TemplateResRef", "LocName", "Description", "Tag", "Comment",
          "Conversation", "Faction", "Appearance", "Type", "AnimationState", "PaletteID",
          ...
          "Plot", "Static", "Useable", "PartyInteract", "HasInventory", "BodyBag", "ItemList",
          "OnUsed", "OnOpen", "OnClosed", "OnDamaged", "OnDeath", "OnDisarm", "OnEndDialogue",
          "OnFailToOpen", "OnHeartbeat", "OnInvDisturbed", "OnLock", "OnMeleeAttacked",
    ...
    seq:
      - id: struct_id
        ...
        doc: |
          ...
          UTP-specific struct IDs:
          - 0xFFFFFFFF (-1): Root struct containing placeable metadata
          - ItemList entries: Structs containing inventory item data (InventoryRes, Repos_PosX, Repos_Posy, Dropable)
      ...
      - id: field_type
        ...
        doc: |
          ...
          0 = Byte (UInt8) - Used for: Lockable, Locked, KeyRequired, AutoRemoveKey, OpenLockDC,
              CloseLockDC, OpenLockDiff, Type, AnimationState, PaletteID, Hardness, Min1HP,
              NotBlastable, Plot, Static, Useable, PartyInteract, HasInventory, BodyBag, Fort,
              Ref, Will, TrapDetectable, TrapDetectDC, TrapDisarmable, DisarmDC, TrapFlag,
              TrapOneShot, TrapType
          1 = Char (Int8)
          ...
          8 = Single (Float32)
          9 = Double (Float64)
          10 = CExoString (String) - Used for: Tag, Comment, KeyName
          11 = ResRef - Used for: TemplateResRef, Conversation, OnUsed, OnOpen, OnClosed,
              OnDamaged, OnDeath, OnDisarm, OnEndDialogue, OnFailToOpen, OnHeartbeat, OnInvDisturbed,
              OnLock, OnMeleeAttacked, OnSpellCastAt, OnTrapTriggered, OnUnlock, OnUserDefined
          12 = CExoLocString (LocalizedString) - Used for: LocName, Description
          15 = List - Used for: ItemList (list of inventory item structs)
          13 = Void (Binary)
          14 = Struct
          15 = List
          16 = Vector4
    ...
    seq:
      - id: length
        ...
        doc: |
          ...
          Common UTP ResRefs: placeable template names (e.g., "crate_001", "container_locked"),
          conversation scripts (e.g., "terminal_conv"), script hooks (e.g., "placeable_used").
    ...
    seq:
      ...
      - id: data
        ...
        doc: |
          ...
          Common UTP strings: Tag (placeable tag for scripting), Comment (developer notes),
    ...
    seq:
      - id: indices
        ...
        doc: |
          ...
          Comment, Conversation, Faction, Type, Lockable, Locked, KeyRequired, HasInventory,
          ItemList, etc.
    ...
    seq:
      - id: raw_data
        ...
        doc: |
          ...
          Note: UTP files use LIST type fields for ItemList (inventory items).
    ...
    seq:
      - id: count
        ...
        doc: |
          ...
          UTP files use LIST type fields for ItemList (inventory items).
    ...
    seq:
      ...
      - id: string_ref
        ...
        doc: |
          ...
          For UTP, LocName and Description use this to reference dialog.tlk entries.
    ...
    seq:
      ...
      - id: string_data
        ...
        doc: |
          ...
          Trailing null bytes should be trimmed.
          For UTP LocName and Description, this contains the placeable name/description

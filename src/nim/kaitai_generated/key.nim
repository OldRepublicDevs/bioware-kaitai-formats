...
proc filename(this: Key_FileEntry): string = 
  ...
  let pos = this.io.pos()
  this.io.seek(int((Key(this.root).fileTableOffset + Key(this.root).bifCount * 12) + this.filenameOffset))
...
The game uses this name to access the resource.
  ...
  let resourceTypeExpr = Key_XoreosFileTypeId(this.io.readU2le())

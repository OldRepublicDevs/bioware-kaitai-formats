...
pt_t::pt_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent, pt_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
...
bool pt_t::file_type_valid() {
    if (f_file_type_valid)
        return m_file_type_valid;
    f_file_type_valid = true;
    m_file_type_valid = gff_data()->header()->file_type() == std::string("PT  ");
    return m_file_type_valid;
}

bool pt_t::version_valid() {
    if (f_version_valid)

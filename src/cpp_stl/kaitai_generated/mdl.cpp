...
#include "mdl.h"
std::set<mdl_t::controller_type_t> mdl_t::_build_values_controller_type_t() {
    std::set<mdl_t::controller_type_t> _t;
    _t.insert(mdl_t::CONTROLLER_TYPE_POSITION);
    _t.insert(mdl_t::CONTROLLER_TYPE_ORIENTATION);
    _t.insert(mdl_t::CONTROLLER_TYPE_SCALE);
    _t.insert(mdl_t::CONTROLLER_TYPE_COLOR);
    _t.insert(mdl_t::CONTROLLER_TYPE_RADIUS);
    _t.insert(mdl_t::CONTROLLER_TYPE_SHADOW_RADIUS);
    _t.insert(mdl_t::CONTROLLER_TYPE_VERTICAL_DISPLACEMENT_OR_DRAG_OR_SELFILLUMCOLOR);
    _t.insert(mdl_t::CONTROLLER_TYPE_ALPHA);
    _t.insert(mdl_t::CONTROLLER_TYPE_MULTIPLIER_OR_RANDVEL);
    return _t;
}
const std::set<mdl_t::controller_type_t> mdl_t::_values_controller_type_t = mdl_t::_build_values_controller_type_t();
bool mdl_t::_is_defined_controller_type_t(mdl_t::controller_type_t v) {
    return mdl_t::_values_controller_type_t.find(v) != mdl_t::_values_controller_type_t.end();
}
std::set<mdl_t::model_classification_t> mdl_t::_build_values_model_classification_t() {
    std::set<mdl_t::model_classification_t> _t;
    _t.insert(mdl_t::MODEL_CLASSIFICATION_OTHER);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_EFFECT);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_TILE);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_CHARACTER);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_DOOR);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_LIGHTSABER);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_PLACEABLE);
    _t.insert(mdl_t::MODEL_CLASSIFICATION_FLYER);
    return _t;
}
const std::set<mdl_t::model_classification_t> mdl_t::_values_model_classification_t = mdl_t::_build_values_model_classification_t();
bool mdl_t::_is_defined_model_classification_t(mdl_t::model_classification_t v) {
    return mdl_t::_values_model_classification_t.find(v) != mdl_t::_values_model_classification_t.end();
}
std::set<mdl_t::node_type_value_t> mdl_t::_build_values_node_type_value_t() {
    std::set<mdl_t::node_type_value_t> _t;
    _t.insert(mdl_t::NODE_TYPE_VALUE_DUMMY);
    _t.insert(mdl_t::NODE_TYPE_VALUE_LIGHT);
    _t.insert(mdl_t::NODE_TYPE_VALUE_EMITTER);
    _t.insert(mdl_t::NODE_TYPE_VALUE_REFERENCE);
    _t.insert(mdl_t::NODE_TYPE_VALUE_TRIMESH);
    _t.insert(mdl_t::NODE_TYPE_VALUE_SKINMESH);
    _t.insert(mdl_t::NODE_TYPE_VALUE_ANIMMESH);
    _t.insert(mdl_t::NODE_TYPE_VALUE_DANGLYMESH);
    _t.insert(mdl_t::NODE_TYPE_VALUE_AABB);
    _t.insert(mdl_t::NODE_TYPE_VALUE_LIGHTSABER);
    return _t;
}
const std::set<mdl_t::node_type_value_t> mdl_t::_values_node_type_value_t = mdl_t::_build_values_node_type_value_t();
bool mdl_t::_is_defined_node_type_value_t(mdl_t::node_type_value_t v) {
    return mdl_t::_values_node_type_value_t.find(v) != mdl_t::_values_node_type_value_t.end();
}
...
void mdl_t::_clean_up() {
    ...
    if (f_animations && !n_animations) {
        if (m_animations) {
            for (std::vector<animation_header_t*>::iterator it = m_animations->begin(); it != m_animations->end(); ++it) {
                delete *it;
            }
...
mdl_t::animation_header_t::animation_header_t(kaitai::kstream* p__io, mdl_t* p__parent, mdl_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = p__root;
    m_geo_header = 0;
...
std::vector<mdl_t::animation_header_t*>* mdl_t::animations() {
    ...
    if (model_header()->animation_count() > 0) {
        ...
        for (int i = 0; i < l_animations; i++) {
            m_animations->push_back(new animation_header_t(m__io, this, m__root));
        }
        m__io->seek(_pos);
    }
    return m_animations;

...
git_t::git_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent, git_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
...
void git_t::_clean_up() {
    if (m_gff_data) {
        delete m_gff_data; m_gff_data = 0;
    }
}

bool git_t::file_type_valid() {
    if (f_file_type_valid)
        return m_file_type_valid;
    f_file_type_valid = true;
    m_file_type_valid = gff_data()->header()->file_type() == std::string("GIT ");
    return m_file_type_valid;
}

bool git_t::version_valid() {
    if (f_version_valid)

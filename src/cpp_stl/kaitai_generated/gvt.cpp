...
gvt_t::gvt_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent, gvt_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
...
void gvt_t::_clean_up() {
    if (m_gff_data) {
        delete m_gff_data; m_gff_data = 0;
    }
}

bool gvt_t::file_type_valid() {
    if (f_file_type_valid)
        return m_file_type_valid;
    f_file_type_valid = true;
    m_file_type_valid = gff_data()->header()->file_type() == std::string("GVT ");
    return m_file_type_valid;
}

bool gvt_t::version_valid() {
    if (f_version_valid)
